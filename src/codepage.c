...
static const struct cp_data cp_data[] = {
	...
	{ 0, 0, 0, 0, 0, 0, 0, 0 }, "CP932,MS_KANJI,SHIFT_JIS,SJIS"
    },
...
static const uint8_t *cp_dbcs = cp_data[0].dbcs;
...
void set_codepage(const char *cp_name)
{
    ...
    for(const struct cp_data *cp = cp_data; cp->names; cp++)
    {
		...
		cp_dbcs = cp->dbcs;
	        if (cp->iconv_name) {
		    cp_iconv_input =
			iconv_open(ICONV_UCS2BE_NAME, cp->iconv_name);
		    cp_iconv_output =
			iconv_open(cp->iconv_name, ICONV_UCS2BE_NAME);
		}
                return;
            }
        }
