...
static const struct cp_data cp_data[] = {
    ...
    {"932,CP932,IBM932,Japanese,ShiftJIS",
     ...
     {0x81, 0x9F, 0xE0, 0xFC, 0, 0, 0, 0},
     "CP932,MS_KANJI,SHIFT_JIS,SJIS"},
...
static void set_codepage_iconv()
{
    if(cp_iconv_name)
    {
        ...
        {
            cp_iconv_input = iconv_open(name, ICONV_UCS2BE_NAME);
            cp_iconv_output = iconv_open(ICONV_UCS2BE_NAME, name);
            if(cp_iconv_input != (iconv_t)(-1) && cp_iconv_output != (iconv_t)(-1))
                break;
...
static int read_codepage_file(const char *fname)
{
    ...
    while(!feof(f))
    {
        ...
        {
            int dbcs[8];
            sscanf(line + 5, "%x %x %x %x %x %x %x %x", dbcs, dbcs + 1, dbcs + 2,
                   dbcs + 3, dbcs + 4, dbcs + 5, dbcs + 6, dbcs + 7);
            for(int i = 0; i < 8; i++)
...
int utf8_to_unicode(const uint8_t **p)
{
    ...
    else if(u[0] < 0xE0)
    {
        ...
        {
            unicode = (((uint16_t)u[0] & 0x1F) << 6) | (u[1] & 0x3F);
            u += 2;
        }
    }
    else if(u[0] < 0xf0)
    {
        ...
        unicode = (((uint16_t)u[0] & 0x0F) << 12) | (((uint16_t)u[1] & 0x3F) << 6) |
                  (u[2] & 0x3F);
        u += 3;
    }
    else
