...
void ems_put8(int addr, int value)
{
    int i;
    if(addr < EMS_ADDR_BEGIN || addr >= EMS_ADDR_END)
    {
...
static struct ems_data **search_handle(unsigned handle)
{
    struct ems_data **pp;
...
static struct ems_data *search_handle_name(uint8_t *name)
{
...
static void validate_emsmap(void)
{
...
static void set_emm_result(uint16_t ax, uint16_t error)
{
...
void intr67(void)
{
    ...
    debug(debug_dos, "D-67%04X: BX=%04X CX:%04X DX:%04X DI=%04X DS:%04X ES:%04X\n", ax,
          cpuGetBX(), cpuGetCX(), cpuGetDX(), cpuGetDI(), cpuGetDS(), cpuGetES());
    ...
    case 0x43: // Get handle and allocate memory
    {
        ...
        *pp = new_ems;
        ...
        cpuSetDX(handle);
        set_emm_result(ax, EMM_STATUS_SUCCESS);
        debug(debug_dos, "EMM Alloc:(%d) %d pages\n", handle, alloc_pages);
    }
    break;
    ...
    case 0x53: // 4.0: Get/set handle name
    {
        ...
        case 0x5301: // Set handle name
        {
            ...
            {
                set_emm_result(ax, EMM_STATUS_HANDLE_NAME_ALREADY_EXIST);
            }
            else
            {
    ...
    case 0x57: // 4.0: Move/exchange memory region
    {
        ...
        else if(src_ems != NULL && dest_ems == NULL)
        { /* ems-to-conv */
            uint32_t dest_addr = cpuGetAddress(dest_pg, dest_offset);
            int end_pg = src_pg + (src_offset + len) / EMS_PAGESIZE;
            if(src_ems->pages < end_pg)
            {
                set_emm_result(ax, EMM_STATUS_EMS_SRC_OR_DEST_IS_OUT_OF_RANGE);
                break;
            }
        ...
        { // ems-to-ems
            ...
            if(src_ems->pages < src_end_pg || dest_ems->pages < dest_end_pg)
            {
                set_emm_result(ax, EMM_STATUS_EMS_SRC_OR_DEST_IS_OUT_OF_RANGE);
                break;
            }
            ...
            if((src_ems == dest_ems) &&
               ((src_offset < dest_offset && (src_offset + len) > dest_offset) ||
                (src_offset > dest_offset && src_offset < (dest_offset + len))))
            {
                overlapped = 1;
            }
            if(exchange)
            {
                ...
                memcpy(&dest_ems->memory[dest_offset], &src_ems->memory[src_offset], len);
                memcpy(&src_ems->memory[src_offset], buf, len);
                free(buf);
            }
            else
            {
                memmove(&dest_ems->memory[dest_offset], &src_ems->memory[src_offset],
                        len);
            }
        }
        set_emm_result(ax, overlapped ? EMM_STATUS_EMS_SRC_AND_DEST_IS_OVERLAPPED_VALID
                                      : EMM_STATUS_SUCCESS);
    }
    break;
    ...
    debug(debug_dos, "R-67%04X: BX=%04X CX:%04X DX:%04X DI=%04X DS:%04X ES:%04X\n",
          cpuGetAX(), cpuGetBX(), cpuGetCX(), cpuGetDX(), cpuGetDI(), cpuGetDS(),
          cpuGetES());
}
